...
//For making the new directory 'LDv4'
#include <sys/types.h>
#include <sys/stat.h>
#include <unistd.h>

//All the arrays that will be stored in the kernel
#include "Exports/LDv4Font.c"
#include "Exports/LDv4Space.c"
#include "Exports/LDv4Engine.c"
#include "Exports/LDv4Cheats.c"
#include "Exports/LDv4Hook.c"
#include "Exports/LDv4Scanner.c"
...
//Settings variables
char *SettFile = "mc0:/LDv4/settings.txt";
...
void load_elf(char *elf_path)
{
	...
	u32 SpaceRead = (u32)LDv4Space;
	for (i = 0; i < sizeof(LDv4Space); i += 1)
	{
		DI();
		ee_kmode_enter();
	...
	u32 FontRead = (u32)LDv4Font;
	for (i = 0; i < sizeof(LDv4Font); i += 1)
	{
		DI();
		ee_kmode_enter();
	...
	u32 HookRead = (u32)LDv4Hook;
	for (i = 0; i < sizeof(LDv4Hook); i += 4)
	{
		DI();
		ee_kmode_enter();
	...
	u32 EngineRead = (u32)LDv4Engine;
	for (i = 0; i < sizeof(LDv4Engine); i += 4)
	{
		DI();
		ee_kmode_enter();
	...
	u32 CheatRead = (u32)LDv4Cheats;
	for (i = 0; i < sizeof(LDv4Cheats); i += 4)
	{
		DI();
		ee_kmode_enter();
	...
	u32 ScannerRead = (u32)LDv4Scanner;
	for (i = 0; i < sizeof(LDv4Scanner); i += 4)
	{
		DI();
		ee_kmode_enter();
	...
	if (AutoJoker == 1 && uLEJoker == 1) {
		...
		char *loc = "mass:/LDv4/LDv4Joker.txt";
		if (fioOpen(loc, O_RDONLY) < 0) { 
			loc = "mc0:/LDv4/LDv4Joker.txt";
			if (fioOpen(loc, O_RDONLY) < 0) {
				loc = "mc1:/LDv4/LDv4Joker.txt";
				if (fioOpen(loc, O_RDONLY) < 0) {
					loc = NULL;
				}
			}
