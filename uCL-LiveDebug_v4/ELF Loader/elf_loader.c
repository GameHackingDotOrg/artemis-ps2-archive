...
int main (int argc, char *argv[1])
{
	...
	GS_BGCOLOUR = purple; /* PURPLE: Launching uLaunchELF */
	load_elf_ram((char*)ule_path);
...
int load_elf_ram(char *elf) {
	...
	if ((!r) && (sifelf.epc)) {
		...
		fioRead(fd, (u32*)elfloc, size); //Reads the elf and stores it into the memory at 0x00100000
		...
		*/
		...
		if (!boot_header->entry) //If the entrypoint is 0 exit and return -3
			return -3;
