...
extern void memory_viewer(void);

extern int j_defhandler;
extern void write_cheats(void) _RESIDENT_;
extern void write_hook(void) _RESIDENT_;
extern u32	*codestack;
...
void HookSyscall(int syscall_num, void *myhandler, u32 *j_defhandler)
{
   ...
   *j_defhandler = MAKE_J(vector);
   printf("MAKE_J(vector) = %8.8x\n",MAKE_J(vector));
	printf("KSEG0(myhandler) = %8.8x\n",KSEG0(myhandler));

   // Patch default vector to call my handler first
   SetSyscall(syscall_num, KSEG0(myhandler));
	#if defined DEBUG
   printf("Hooked syscall 0x%02X (old vector: 0x%8.8X, current: 0x%8.8X)\n",
...
int HelloWorld(void)
{
	__asm("fuckinghere1:");
	__asm("nop:");
	g2_fill_rect(30, 35, 100, 101, 0xFF, 0xFE, 0xFD);
	return 1;
}
/////////////////////////////////////////////////////////////////////
//							MAIN FUNCTION
...
int main(void)
{
	...
	//welcome message
	scr_printf("				writen by cYs Driver\n");
	scr_printf("	Press X to load cheats \n");
	...
	//create jal to write_cheats
	_sw(MAKE_JAL(&write_cheats), &jal_cheats);
	// = ;
	#if defined DEBUG
	...
	while(1)
	{
		...
		if(new_pad & PAD_CROSS){
			//lable Cross
			__asm("cross:");
			//hook the syscall - systemcall number, write_hook address where it places jal on padread - location at the end of my function
			HookSyscall(syscall_num, &write_hook, &j_defhandler);
			scr_printf("	Done\n\n");
			write_cheats();
		}
