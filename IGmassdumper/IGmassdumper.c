...
void SendIrxKernelRAM(void)
{
	...
	irxptr_tab[2].irxsize = size_usbd_irx;
	irxptr_tab[3].irxsize = size_usb_mass_irx;
	irxptr_tab[4].irxsize = size_usbserv_irx;
	 	
	irxsrc[0] = (void *)&memdisk_irx;
	irxsrc[1] = (void *)&eesync_irx;
	irxsrc[2] = (void *)&usbd_irx;	
	irxsrc[3] = (void *)&usb_mass_irx;
	irxsrc[4] = (void *)&usbserv_irx;
...
{	
	init_scr();
	scr_clear();
	scr_printf("\t InGame Mass Dumper - v0.1\n\n");
